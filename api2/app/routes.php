...
return function (App $app) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
    ...
    $app->post('/quiz/create', function (Request $request, Response $response, $args) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
        ...
        try {
            ...
            // log $DB_DSN, $DB_USER, $DB_PASSWORD  
            error_log($DB_DSN);
            error_log($DB_USER);
    ...
    $app->post('/quiz/delete', function (Request $request, Response $response, $args) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
    ...
    $app->post('/quiz/prepare_correction', function (Request $request, Response $response, $args) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
        // save from request body
        $data = $request->getParsedBody();
        $quiz_name = $data['name'];
    ...
    $app->post('/quiz/uploadlist', function (Request $request, Response $response, $args) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
    ...
    $app->post('/quiz/uploadcorrect', function (Request $request, Response $response, $args) use ($DB_DSN, $DB_USER, $DB_PASSWORD) {
        $quiz_name = $request->getParsedBody()['name']; // nombre total de chunks
        $directory = __DIR__ . "/../quiz_data/" . $quiz_name; // répertoire de destination de l'upload
        $resumableIdentifier = $request->getParsedBody()['resumableIdentifier']; // identifiant de l'upload
        $resumableChunkNumber = $request->getParsedBody()['resumableChunkNumber']; // numéro de chunk
