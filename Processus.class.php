...
class Processus {
	...
	public static function createBackgroundProcess($quiz, $command) {
		global $quiz_bin_dir;
		$time = time();
		touch("C:/wamp64/www/QCM_project/home/quiz/tmp/".$quiz->getName().".stdout");
		touch("C:/wamp64/www/QCM_project/home/quiz/tmp/".$quiz->getName().".stderr");
		echo "<html><head>la valeur de command est $command </head></html>";
		$pid = exec($command." > C:/wamp64/www/QCM_project/home/quiz/tmp/".$quiz->getName().".stdout 2> C:/wamp64/www/QCM_project/home/quiz/tmp/".$quiz->getName().".stderr & PID=\$!; echo \$PID; ".$quiz_bin_dir."wait_process.sh ".$quiz->getName()." \$PID > C:/wamp64/www/QCM_project/home/quiz/tmp/wait.txt &");
		$f = fopen("C:/wamp64/www/QCM_project/home/quiz/tmp/".$quiz->getName().".info", "w");
		fwrite($f, $pid."\n");
		fwrite($f, $command."\n");
		fwrite($f, $time."\n");
		fclose($f);
